...
- **朝向自查**：模型检视图第一格“朝向 [1,0]”里，`+x` 指向画面**右下方**。车头、机头、船首必须出现在第一格的右下侧，炮管、尾翼、尾桨在左上侧。提交前先看这一点。
...
- `models3d.js` 末尾为每张卡预留了一段 `// ---------- T-xx ... ----------`：**只在自己那一段里写代码**，写完用 `MODEL3D.类型名 = 函数;` 注册。范本：车辆看 `m3T64`，人物用 `soldier3()`，建筑看 `m3House`，树用 `pine3()` / `birch3()`，飞机和流线型机身看 `m3Ka52` / `m3Mi8`（用 `lathe()` 做机身、`rotor()` 做旋翼）。
